...
class ClientV2 {
    ...
    splitDataForExternalCall(data, usePriorityFee = false) {
        const INIT_IX_MAX_SIZE = 880 - (usePriorityFee ? 70 : 0);
        const EXTEND_IX_MAX_SIZE = 876 - (usePriorityFee ? 70 : 0);
        const result = [];
        if (data.length <= INIT_IX_MAX_SIZE) {
            return [{ data, offset: 0 }];
        }
        let currentPos = 0;
        while (currentPos < data.length) {
            const chunkSize = currentPos == 0 ? INIT_IX_MAX_SIZE : EXTEND_IX_MAX_SIZE;
    ...
    async buildFillExtcallStorageTransactions(sourceChain, totalLength, chunks, storageKey, accounts, priorityFee) {
        ...
        if (chunks.length === 0)
            return null;
        const initPriorityFee = (_a = priorityFee === null || priorityFee === void 0 ? void 0 : priorityFee.init) !== null && _a !== void 0 ? _a : (_b = this.priorityFeeConfig) === null || _b === void 0 ? void 0 : _b.initExternalCall;
        const storePriorityFee = (_c = priorityFee === null || priorityFee === void 0 ? void 0 : priorityFee.store) !== null && _c !== void 0 ? _c : (_d = this.priorityFeeConfig) === null || _d === void 0 ? void 0 : _d.storeExternalCall;
        ...
        for (const chunk of chunks) {
            if (chunk.offset === 0) {
                result.init = {
                    instructions: newTxWithOptionalPriorityFee(initPriorityFee).add(await _1.instructions
                        .initExternalCallStorageInstruction(this.program, ixContext, {
                ...
                result.store.push({
                    instructions: newTxWithOptionalPriorityFee(storePriorityFee).add(await _1.instructions
                        .updateExternalCallStorageInstruction(this.program, ixContext, {
                        rawInstructions: chunk,
                        sourceChainId,
    ...
    async prepareExternalCallTransactions(storageKey, sourceChain, data, accounts, priorityFee) {
        ...
        const initPriorityFee = (_a = priorityFee === null || priorityFee === void 0 ? void 0 : priorityFee.init) !== null && _a !== void 0 ? _a : (_b = this.priorityFeeConfig) === null || _b === void 0 ? void 0 : _b.initExternalCall;
        const storePriorityFee = (_c = priorityFee === null || priorityFee === void 0 ? void 0 : priorityFee.store) !== null && _c !== void 0 ? _c : (_d = this.priorityFeeConfig) === null || _d === void 0 ? void 0 : _d.storeExternalCall;
        const logger = (_e = this.logger) === null || _e === void 0 ? void 0 : _e.child({ method: "prepareExternalCallTransactions" });
        if (((_f = accounts.submission) === null || _f === void 0 ? void 0 : _f.data) !== null) {
        ...
        if (accounts.externalCallMeta) {
            ...
            if (!(0, interfaces_1.isExtCallMetaAccumulation)((_g = decodedExtCallMeta === null || decodedExtCallMeta === void 0 ? void 0 : decodedExtCallMeta.data) !== null && _g !== void 0 ? _g : null)) {
                ...
                if (((_h = accounts.externalCallStorage) === null || _h === void 0 ? void 0 : _h.data) === undefined || accounts.externalCallStorage.data === null) {
                    throw new Error(`storage should be already initialized, something is wrong`);
                }
                else if (data.equals((_j = accounts.externalCallStorage.data) === null || _j === void 0 ? void 0 : _j.slice(constants_1.EXT_CALL_STORAGE_OFFSET))) {
            ...
            chunksToWrite = this.splitDataForExternalCall(data, initPriorityFee !== undefined || storePriorityFee !== undefined);
        }
    ...
    tryPackCalldataTransaction(ixs, extCallTxs) {
        var _a;
        const result = new web3_js_1.Transaction().add(...ixs.prefix);
        if (extCallTxs) {
            const txCopy = new web3_js_1.Transaction(result);
            let lastIx = null;
            if (extCallTxs.init && extCallTxs.store.length === 0) {
                lastIx = extCallTxs.init.instructions.at(-1);
                if (lastIx === undefined)
                    throw new Error(`Unreachable, empty init ext call tx`);
            }
            else if (extCallTxs.init === undefined && extCallTxs.store.length === 1) {
                lastIx = (_a = extCallTxs.store.at(0)) === null || _a === void 0 ? void 0 : _a.instructions.at(0);
            }
    ...
    buildSendContext(sender, tokenMint, chainTo, externalCall, options) {
        ...
        const [externalCallStorage] = this.accountsResolver.getExternalCallStorageAddress(shortcut, sender, solana_utils_1.crypto.normalizeChainId(solana_utils_1.constants.SOLANA_CHAIN_ID));
    ...
    async buildSendTransactions(sender, amount, receiverInfo, senderInfo, autoParams, options) {
        ...
        if (externalCallExists) {
            ...
            extCallTxs = await this.prepareExternalCallTransactions(externalCallShortcut, solana_utils_1.constants.SOLANA_CHAIN_ID, externalCallData, {
                executor: sender,
