...
class DeBridgeSolanaClient {
    constructor(connection, wallet, params) {
        this.debug = false;
        this.walletProvided = wallet != undefined;
    ...
    async getSubmissionStatus(submissionId) {
        ...
        try {
            ...
            const transactions = await this._connection.getConfirmedSignaturesForAddress2(extCallMeta, { limit: 100 });
    ...
    */
    ...
    async calculateExecutionFee(senderAddressLength, bridgeId, solPrice, tokenPrice, tokenDecimals, walletExists, executionFeeMultiplier, isRequiredTempRentCost, extCall) {
        ...
        const costInput = new wasm.CostCalculationInput(senderAddressLength, isRequiredTempRentCost, guard.excessConfirmations, BigInt(rent.lamportsPerByteYear), rent.exemptionThreshold, signaturePrice, solPrice, tokenPrice, tokenDecimals, bridge !== null, walletExists, executionFeeMultiplier - 1, extCall ? BigInt(extCall.length) : undefined);
        const weiPrice = BigInt(await costInput.calculate_recomended_claim_execution_fee());
        costInput.free();
        const rewards = [];
        if (extCall) {
            const solanaDimensionCostInput = new wasm.CostCalculationInput(senderAddressLength, isRequiredTempRentCost, guard.excessConfirmations, BigInt(rent.lamportsPerByteYear), rent.exemptionThreshold, signaturePrice, solPrice, tokenPrice, this.getWrapperDecimals(tokenDecimals), bridge !== null, walletExists, executionFeeMultiplier - 1, extCall ? BigInt(extCall.length) : undefined);
    ...
    async buildInitializeSendBridgeTransaction(tokenMint, payer) {
        ...
        const bridgeId = solana_utils_1.helpers.hexToBuffer(solana_utils_1.crypto.hashDebridgeId(solana_utils_1.constants.SOLANA_CHAIN_ID, tokenMint.toBuffer()));
    ...
    async buildSendContext(sender, sendFromWallet, tokenMint, receiver, chainIdTo, useAssetFee, fallbackAddress, flags, data) {
        ...
        const [externalCallStorage] = this.accountsResolver.getExternalCallStorageAddress(externalCallShortcut, sender, solana_utils_1.crypto.normalizeChainId(solana_utils_1.constants.SOLANA_CHAIN_ID));
        const [externalCallMeta] = this.accountsResolver.getExternalCallMetaAddress(externalCallStorage);
        const accounts = {
            bridgeCtx: {
                bridge,
    ...
    async buildSendInstruction(sender, sendFromWallet, payableAmount, tokenMint, receiver, chainIdTo, useAssetFee, referralCode, fallbackAddress, flags, executionFee, data, checkSenderBalance = true, sendFromWalletMayNotExist = false) {
        ...
        if (data && !(0, utils_1.checkFlag)(flags, constants_1.SEND_HASHED_DATA)) {
            exTxs = await this.prepareExtCallTransactions(externalCallShortcut, solana_utils_1.constants.SOLANA_CHAIN_ID, sender, data, {
        ...
        const sendIx = await instructions
            ...
            .instruction();
        let sendIxUsed = false;
        if (exTxs.init) {
            const sizeCheckTx = new web3_js_1.Transaction().add(result).add(sendIx);
            sizeCheckTx.add(...exTxs.init.instructions);
            const txSize = solana_utils_1.txs.getTransactionSize(sizeCheckTx);
            loglevel_1.default.debug(`[sendIx] Concat tx size: ${txSize !== null && txSize !== void 0 ? txSize : "too big"}, ${txSize ? solana_utils_1.constants.MAX_TX_SIZE - txSize : ""}`);
            if (txSize != null && txSize <= 1230) {
                result.add(...exTxs.init.instructions);
                result.add(sendIx);
                sendIxUsed = true;
                exTxs.init = undefined;
            }
        }
        if (!sendIxUsed)
            result.add(sendIx);
    ...
    async hashSubmissionId(params) {
        ...
        if ((0, interfaces_1.isMintBridge)(bridgeData)) {
            ...
            debridgeId = solana_utils_1.helpers.hexToBuffer(solana_utils_1.crypto.hashDebridgeId(solana_utils_1.constants.SOLANA_CHAIN_ID, nativeTokenAddress)); // correct
        }
        else {
            throw new Error(`Bridge not supported!`);
        }
        const subId = solana_utils_1.crypto.hashSubmissionIdRaw({
            ...
            targetChainId: solana_utils_1.constants.SOLANA_CHAIN_ID,
     ...
     * @param fullDataSize size of ext call data length
     * @returns init ext call storage instruction
     */
    async buildInitExternalCallInstruction(submissionId, sourceChainId, executor, calculatedAccounts, data, fullDataSize, priorityFee) {
        var _a;
        executor = new web3_js_1.PublicKey(executor);
        ...
        const [extCallStorage] = (calculatedAccounts === null || calculatedAccounts === void 0 ? void 0 : calculatedAccounts.storage)
            ...
            : this.accountsResolver.getExternalCallStorageAddress(subIdBuffer, executor, chainId);
        ...
        const totalLen = fullDataSize ? fullDataSize : (data === null || data === void 0 ? void 0 : data.length) || 0;
        const ix = await instructions
            .initExternalCallStorageInstruction(this._program, {
            externalCallMeta: extCallMeta,
            externalCallStorage: extCallStorage,
            claimer: executor,
        }, {
            sourceChainId: chainId,
            storageKey: subIdBuffer,
            externalCallLen: totalLen,
            rawInstructions: data ? data : buffer_1.Buffer.from([]),
        })
            .instruction();
        const tx = newTxWithOptionalPriorityFee(priorityFee !== null && priorityFee !== void 0 ? priorityFee : (_a = this.priorityFeeConfig) === null || _a === void 0 ? void 0 : _a.initExternalCall);
        tx.add(ix);
        return { instructions: tx.instructions, payer: executor };
    }
    /**
     ...
     */
    async buildExtendExternalCallStorageInstruction(submissionId, sourceChainId, executor, data, calculatedAccounts, priorityFee) {
        var _a;
        executor = new web3_js_1.PublicKey(executor);
        const subIdBuffer = isBuffer(submissionId) ? submissionId : solana_utils_1.helpers.hexToBuffer(submissionId);
        const chainId = solana_utils_1.crypto.normalizeChainId(sourceChainId);
        ...
        const ix = await instructions
            .updateExternalCallStorageInstruction(this._program, { claimer: executor, externalCallStorage, externalCallMeta }, { submissionId: subIdBuffer, sourceChainId: chainId, rawInstructions: data })
            .instruction();
    ...
    static splitDataForExternalCall(data) {
        const result = [];
        if (data.length <= CALLDATA_CHUNK_SIZE) {
            return [{ data, offset: 0 }];
        }
        let currentPos = 0;
        while (currentPos < data.length) {
            const chunk = data.slice(currentPos, currentPos + CALLDATA_CHUNK_SIZE);
    ...
    async prepareExtCallTransactions(submissionId, sourceChainId, executor, data, calculatedAccounts) {
        ...
        const [extCallStorage] = (calculatedAccounts === null || calculatedAccounts === void 0 ? void 0 : calculatedAccounts.storage)
            ? [calculatedAccounts.storage]
            : this.accountsResolver.getExternalCallStorageAddress(isBuffer(submissionId) ? submissionId : solana_utils_1.helpers.hexToBuffer(submissionId), executor, chainId);
        const [extCallMeta] = (calculatedAccounts === null || calculatedAccounts === void 0 ? void 0 : calculatedAccounts.meta)
            ? [calculatedAccounts.meta]
            : this.accountsResolver.getExternalCallMetaAddress(extCallStorage);
        ...
        if (storedData === null) {
            ...
            const firstChunk = dataChunks.shift();
            const transactions = [];
            const initExtCall = await this.buildInitExternalCallInstruction(submissionId, chainId, executor, {
                meta: extCallMeta,
                storage: extCallStorage,
            }, (firstChunk === null || firstChunk === void 0 ? void 0 : firstChunk.data) || buffer_1.Buffer.from([]), totalSize);
            for (const chunk of dataChunks) {
                transactions.push(await this.buildExtendExternalCallStorageInstruction(submissionId, sourceChainId, executor, chunk, {
                    extCallStorage,
                    extCallMeta,
                }));
            }
            return { ...result, init: { instructions: initExtCall.instructions, payer: executor }, store: transactions };
        }
        else {
            ...
            if (storedData === null || storedData === void 0 ? void 0 : storedData.data.slice(constants_1.EXT_CALL_STORAGE_OFFSET).equals(data)) {
                ...
                while (offset < data.length) {
                    ...
                    if (!onChainData.slice(...range).equals(correctChunk)) {
                        ...
                        const updateExtCallIx = await this.buildExtendExternalCallStorageInstruction(submissionId, sourceChainId, executor, { data: correctChunk, offset }, {
                            extCallMeta,
                            extCallStorage,
                        });
    ...
    async storeExternallCall(extCallTransactions, sender, data, overwriteOnError = false) {
        var _a;
        if (extCallTransactions.init === undefined && extCallTransactions.store.length === 0)
            return [];
        data = buffer_1.Buffer.isBuffer(data) ? data : solana_utils_1.helpers.hexToBuffer(data);
        let result = [];
        const firstTx = (_a = extCallTransactions.init) !== null && _a !== void 0 ? _a : extCallTransactions.store[0];
        ...
        // alloc storage only when allocated == null
        if (extCallTransactions.init) {
            if (onChainData !== null) {
                throw new Error("Calldata already allocated");
            }
            let retries = 0;
            if (!onChainData) {
                const [initId] = await sender(extCallTransactions.init);
                loglevel_1.default.debug(`Initialize extCall id: ${initId}`);
                result.push(initId);
                const isEmpty = (acc) => (acc === null ? true : acc.lamports === 0);
                for (;;) {
                    const [meta, storage] = await this._connection.getMultipleAccountsInfo([metaAccount, storageAccount]);
                    if (!isEmpty(meta) && !isEmpty(storage))
                        break;
                    await solana_utils_1.helpers.sleep(3000);
                    retries += 1;
                    if (retries > 20) {
                        return Promise.reject("Failed to allocate externalCallStorage and extCallMeta");
                    }
                }
            }
        }
    ...
    async storeExternalCallV2(extCallTransactions, data, submissionState, sender, options) {
        ...
        if ((_c = (_b = submissionState.externalCallStorage) === null || _b === void 0 ? void 0 : _b.isEmpty) !== null && _c !== void 0 ? _c : true) {
            if (extCallTransactions.init === undefined) {
                throw new Error("Init ext call transaction missing while storage is not initialized. Maybe some problems with the rpc?");
            }
            const allocPromise = new Promise((resolve, reject) => {
                ...
                submissionState.onActionOnce(Submission.UpdateActions.ExternalCallStorage, (onChainData) => {
                    ...
                    resolve(onChainData !== null && onChainData.slice(constants_1.EXT_CALL_STORAGE_OFFSET).equals(data));
                });
            });
            const [initId] = await sender(extCallTransactions.init);
            loglevel_1.default.debug(`Initialize extCall id: ${initId}`);
        ...
        if (storedFullDataInAllocTx || extCallTransactions.store.length === 0) {
            return result;
        }
        else {
    ...
    async buildClaimTransaction(executor, amount, receiverInfo, senderInfo, submissionInfo, autoParams, createMissingWallets, confirmationStorageCreator, priorityFee) {
        ...
        let claimInstrUsed = false;
        if (exTxs.init) {
            const sizeCheckTx = new web3_js_1.Transaction().add(result).add(claimInstr);
            sizeCheckTx.add(...exTxs.init.instructions);
            try {
                const txSize = solana_utils_1.txs.getTransactionSize(sizeCheckTx);
                loglevel_1.default.debug(`[claimIx] Concat tx size: ${txSize !== null && txSize !== void 0 ? txSize : "too big"}, space left: ${txSize ? solana_utils_1.constants.MAX_TX_SIZE - txSize : ""}`);
                if (txSize != null && txSize <= 1230) {
                    result.add(...exTxs.init.instructions);
                ...
                // failed to get tx size => tx too big, do nothing
            }
        }
        if (!claimInstrUsed)
            result.add(claimInstr);
